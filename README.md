...
The figure-generation script (`scripts/analysis/build_pdv_inverse_final_figures.py`) produces the following, but the rendered images are generated artifacts under a gitignored `benchmarks/` directory and are **not currently committed to this repository**:

- `mahalanobis_81d_ood_separation.png` — histogram of squared Mahalanobis distance for in-domain calibration vs. chronological validation/test states.
- `repricing_rmse_by_maturity.png` — held-out repricing RMSE (bp of forward) by option maturity.
- `fresh_seed_repricing_robustness.png` — mean repricing RMSE by independent Monte Carlo seed.
- `parameter_recovery_{beta0,beta1,beta2,theta1,theta2}.png` — true-vs-predicted scatter per parameter.

To surface them here, copy small (documentation-sized) exports into `docs/assets/` using the filenames above — see [`docs/assets/README.md`](docs/assets/README.md) for the exact list this README is wired to reference.
