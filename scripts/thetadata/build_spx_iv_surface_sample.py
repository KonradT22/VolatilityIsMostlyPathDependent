...
for expiration, dte, target in selected:
    ...
    raw_path = RAW_DIR / f"spx_surface_raw_{trade_date}_{time_of_day.replace(':','')}_exp_{expiration}.csv"
...
surface_path = OUT_DIR / f"spx_iv_surface_sample_{trade_date}_{time_of_day.replace(':','')}.csv"
summary_path = OUT_DIR / f"spx_iv_surface_summary_{trade_date}_{time_of_day.replace(':','')}.csv"
plot_path = OUT_DIR / f"spx_iv_surface_sample_{trade_date}_{time_of_day.replace(':','')}.png"
...
for dte, group in clean.groupby("dte"):
    ...
    plt.scatter(group["log_moneyness"], group["iv"], label=f"{int(dte)} DTE", s=18)

plt.xlabel("log(K / F)")
plt.ylabel("Black-Scholes implied volatility")
plt.title("SPX OTM IV Surface Sample: 2021-06-02 15:55")
plt.grid(True)
plt.legend()
plt.tight_layout()
